...
<body>
	<% if(member==null) {//아이디 사용이 가능한 경우 %>
		<div>입력한 <span class="memId">[<%=memId %>]</span>는 사용 가능한 
		아이디 입니다.</div>
		<div><button type="button" onclick="windowClose();">아이디 사용</button></div>
	<% } else {//아이디 사용이 불가능한 경우 %>
		<div id="message">입력한 <span class="memId">[<%=memId %>]</span>는
		이미 사용중인 아이디 입니다.<br>
...
<script type="text/javascript">
	function windowClose() {
		//opener : 부모창을 나타내는 객체
		opener.joinForm.memId.value="<%=memId%>";
	...
	function submitCheck() {
		...
		var idReg=/^(?=.*[a-zA-Z])(?=.*[0-9]).{4,16}$/g;
		if(!idReg.test(id)) {
			document.getElementById("message").innerHTML="영문소문자/숫자, 4~16자리로 입력해 주세요.";
			document.getElementById("message").style="color: red;";
			return false;
		}
		return true;
