 ...
 */
function processEasypayPay(sPgName, form, callbackName) {
    var return_data = {
        sync: true,
        result: false,
        msg: ""
    ...
    if (typeof callbackName === "function") {
        PGMODULE_Easypay.setCallback(callbackName);
    } else {
    ...
    if (sPgName === null) {
        ...
        PGMODULE_Easypay.sendCallback(return_data);
    }

    eval("var funcType = typeof PGMODULE_" + sPgName + "_easypay.procPay");
    if (funcType == "function") {
        eval("return_data = PGMODULE_" + sPgName + "_easypay.procPay(form);");
