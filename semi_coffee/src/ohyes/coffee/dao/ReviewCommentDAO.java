...
public class ReviewCommentDAO extends JdbcDAO {
	...
	public int updateCommentReview(int coNo, String coContent, int no) {
		Connection con=null;
		PreparedStatement pstmt=null;
		int rows=0;
		try {
			...
			String sql="update review_comment set review_co_date=sysdate, review_co_content=? where review_co_no=? and review_no=?";
			pstmt=con.prepareCall(sql);
	...
	public ReviewCommentDTO selectCoNoComment(int coNo) {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		ReviewCommentDTO comment=null;
		try {
			con=getConnection();
			
			String sql="select * from review_comment where review_co_no=?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, coNo);
			...
			System.out.println("[에러]selectCoNoComment() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return comment;
	...
	public int deleteComment(int coNo) {
		...
		try {
			...
			String sql="delete review_comment where review_co_no=?";
			pstmt=con.prepareCall(sql);
			pstmt.setInt(1, coNo);
