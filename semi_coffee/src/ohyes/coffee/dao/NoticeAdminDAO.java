...
public class NoticeAdminDAO extends JdbcDAO {
	...
	public List<NoticeDTO> selectNoticeAdminAll(){
		...
		try {
			...
			System.out.println("[에러] selectNoticeAdminAll() 메소드의 SQL 오류 : " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
	...
	public NoticeDTO selectNoticeAdminNo(int noticeNo){
		...
		try {
			...
			System.out.println("[에러] selectNoticeAdminNo() 메소드의 SQL 오류 : " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
	...
	public List<NoticeDTO> selectNoticeAdminByKeyword(int startRow, int endRow, String keyword, final int status ){
		...
		try {
			...
			//select * from (select rownum rn, temp.* from (select * from notice where notice_title like '%'||'공'||'%') temp) where rn between 1 and 10;
			if(keyword.equals("")){
				sql1+=sql2;
			...
			System.out.println("[에러] selectNoticeAdminByKeyword() 메소드의 SQL 오류 : " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
