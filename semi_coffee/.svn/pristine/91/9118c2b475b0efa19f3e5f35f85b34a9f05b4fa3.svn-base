...
<%@include file="/coffee/Mypage/login_check.jspf" %>   
 ...
 <%
	...
	if(loginMember.getMemId() == null){
 	...
 	//--------장바구니에서 넘어오는 경우
 	//1.모든상품이 넘어오는 경우 => 해당 회원의 장바구니 테이블을 모두 가져온다
 	//List<CartDTO> cartList = CartDAO.getDAO().selectAllCart(loginMember.getMemId());//여기에는 회원아이디 /상품번호 /상품수량
 	
 	String[] checkedPno = request.getParameterValues("checkedCart");//상품번호받아옴
 	int size = checkedPno.length;
 	
 	String memId = loginMember.getMemId();
 	//CartOrderDTO cartOrder = null;
 	int totalCost = 0;
 	CartOrderDTO cartOrder = null;
 	
 	for(String pno : checkedPno){
 		//System.out.println(pno);
 		cartOrder = OrderPayDAO.getOrderPayDAO().selectCartToPay(memId, Integer.parseInt(pno));
 		totalCost += cartOrder.getpCost() * cartOrder.getCartAmount();
 	}
...
<div class="tbl_head03 tbl_wrap">
	...
	<%for(String pno : checkedPno){ %>
		<%CartOrderDTO cart = OrderPayDAO.getOrderPayDAO().selectCartToPay(memId, Integer.parseInt(pno)); %>
		<tr>
			<th><%= cart.getpName()%></th>
			
			<th><img src = "<%=request.getContextPath()%>/coffee/admin/product_images/<%= cart.getpImage()%>" width = "100px"/></th>
			
			<th><%=cart.getCartAmount()%></th>
			
			<th><%=cart.getCartAmount() * cart.getpCost()%></th>
...
<form id = "PayForm" method = "post" action = "<%=request.getContextPath()%>/coffee/index.jsp?workgroup=MyPage&work=mypage">
	...
	<input type = "hidden" id = "cartOrder" name = "cartOrder" value = "<%=cartOrder%>">
	<%int i = 0; %>
	<%for(String pno : checkedPno){	%>
 		<%cartOrder = OrderPayDAO.getOrderPayDAO().selectCartToPay(memId, Integer.parseInt(pno));%>
 		<%i++; %>
 		<input type = "hidden" id = "listSize" name = "listSize" value = "<%=checkedPno.length %>">
		<input type = "hidden" id = "pno<%=i%>" name = "pno<%=i%>" value = "<%=cartOrder.getpNo()%>">
		<input type = "hidden" id = "pname<%=i%>" name = "pname<%=i%>" value = "<%=cartOrder.getpName()%>">	
		<input type = "hidden" id = "quantityNum<%=i%>" name = "quantityNum<%=i%>" value = "<%=cartOrder.getCartAmount()%>">
		<!-- 해당 상품의 가격 -->
		<input type = "hidden" id= "cost<%=i%>" name = "cost<%=i%>" value = "<%=cartOrder.getpCost()%>">
...
<strong>동의함</strong></label></p>
		...
		<input id="total_price" name="total_price" fw-filter="isFill" fw-label="결제금액" fw-msg="" class="inputTypeText"
		   placeholder="" style="text-align:right;ime-mode:disabled;clear:none;border:0px;float:none;" size="10" readonly="1" value="<%=totalCost%>" type="text">
		   <span>원</span></p>
...
$("input:text[numberOnly]").on("keyup", function() {
	...
	$("#btn_payment").click(function() {
		...
		$("#PayForm").attr("action", "<%=request.getContextPath()%>/coffee/index.jsp?workgroup=pay&work=payCart_add_action");
		PayForm.submit();
