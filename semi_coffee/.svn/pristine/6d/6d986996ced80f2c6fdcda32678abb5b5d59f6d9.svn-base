...
<%
	...
	//총 게시글 갯수 계산하여 저장
	int totalBoard=ReviewDAO.getDAO().selectReviewCount(search, keyword, category);
            ...
            <div class="board-wrap">
                <div class="xans-board-listpackage">
                    ...
                    <div class="ec-base-table typeList gBorder">
                    	<!-- ****스타일 수정**** -->
						<form method="post" id="categoryForm" action="<%=request.getContextPath()%>/coffee/index.jsp?workgroup=board&work=review_list">
                            ...
                            <tbody>
                            	...
                            	<% } else { %>
	                                ...
	                                <tr style="background-color:#FFFFFF; color:#555555;">
	                                    ...
	                                    <% if(review.getReviewStatus()!=0) { //일반글 %>
	                                    <td class="thumb left">
	                                        <a href="<%=request.getContextPath()%>/coffee/index.jsp?workgroup=product&work=product_detail&no=<%=review.getReviewPNo()%>">
	                                        	<img src="<%=request.getContextPath()%>/coffee/admin/product_images/<%=product.getpImage()%>" border="0" alt="" />
	                                        	<span><%=product.getpName() %></span>
	                                        </a>
	                                    </td>
	                                    <td class="subject left txtBreak" style="padding-left:30px;">
	                                    ...
	                                    <td class="center"><%=review.getReviewMemid() %></td>
	                                    <% if(currentDate.equals(review.getReviewDate().substring(0, 10))) { %>
	                                    <td class="center"><span class="txtNum"><%=review.getReviewDate().substring(11, 16) %></span></td>
	                                    <% } else { %>
	                                    <td class="center"><span class="txtNum"><%=review.getReviewDate().substring(0, 16) %></span></td>
	                                    <% } %>
	                                    <td class="center"><span class="txtNum"><%=review.getReviewReadcount() %></span></td>
	                                    <% } else {//삭제글 %>
	                                    <td class="subject left txtBreak" colspan="6" style="height:50px;">
